...
const { traduzirRespostasDiagnostico, nomePerfil } = require('../lib/traduzir-diagnostico.js');
...
function renderRespostasDiagnostico(respostas) {
  const linhas = traduzirRespostasDiagnostico(respostas);
  if (!linhas.length) return '<span class="empty">-</span>';
  const grupos = {};
  linhas.forEach(l => {
    if (!grupos[l.grupo]) grupos[l.grupo] = [];
    grupos[l.grupo].push(l);
  });
  return Object.keys(grupos).map(grupo => {
    const itens = grupos[grupo].map(l =>
      '<div class="qa-item"><div class="qa-q">' + esc(l.pergunta) + '</div><div class="qa-a">' + esc(l.resposta) + '</div></div>'
    ).join('');
    return '<div class="qa-grupo"><h4>' + esc(grupo) + '</h4>' + itens + '</div>';
  }).join('');
}
...
function page(diagnosticos, mapas) {
  const diagRows = diagnosticos.map(function(d) {
    ...
    '<td>' + (esc(nomePerfil(d.perfil_dominante)) || '-') + '</td>' +
    '<td>' + (d.maturidade_score != null ? d.maturidade_score : '-') + '</td>' +
    '<td>' + (d.saude_score != null ? d.saude_score : '-') + '</td>' +
    '<td><button class="btn-ver" onclick="toggle(\'d-' + d.id + '\')">Ver detalhes</button> <a class="btn-docx" href="/api/admin-export?type=diagnostico&id=' + d.id + '">Baixar DOCX</a> <button class="btn-del" data-type="diagnostico" data-id="' + d.id + '">Apagar</button></td></tr>' +
    '<tr class="detail-row" id="d-' + d.id + '" style="display:none"><td colspan="6">' + renderRespostasDiagnostico(d.respostas) + '</td></tr>';
  }).join('');
  ...
  '.btn-del{font-family:Inter,sans-serif;font-size:12.5px;font-weight:600;color:#fff;background:#C0392B;border:none;padding:7px 14px;border-radius:100px;cursor:pointer;}' +
  ...
  '.empty{color:#999;}' +
  '.qa-grupo{margin-bottom:18px;}' +
  '.qa-grupo h4{font-family:Fraunces,serif;font-size:14px;color:var(--gold);margin:0 0 8px;text-transform:uppercase;letter-spacing:.03em;}' +
  '.qa-item{margin-bottom:10px;padding-bottom:10px;border-bottom:1px dashed var(--line);}' +
  '.qa-item:last-child{border-bottom:none;}' +
  '.qa-q{font-size:13px;color:var(--slate);margin-bottom:3px;}' +
  '.qa-a{font-size:14.5px;color:#1C2430;font-weight:600;}' +
  '</style></head><body>' +
  ...
  'if(!confirm("Tem certeza que deseja apagar esta resposta? Essa acao nao pode ser desfeita.")) return;' +
  'var type = btn.dataset.type, id = btn.dataset.id;' +
  'btn.disabled = true; btn.textContent = "Apagando...";' +
  'fetch("/api/admin-delete", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({type:type, id:id}) })' +
  '.then(function(res){ return res.json().then(function(data){ return {ok:res.ok, data:data}; }); })' +
  '.then(function(result){' +
  'if(result.ok){' +
  'var prefix = type==="diagnostico" ? "d-" : "m-";' +
  'var detailRow = document.getElementById(prefix+id);' +
  'if(detailRow) detailRow.remove();' +
  'btn.closest("tr").remove();' +
  '} else {' +
  'alert("Erro ao apagar: " + (result.data && result.data.error ? result.data.error : "tente novamente."));' +
  'btn.disabled = false; btn.textContent = "Apagar";' +
  '}' +
  '})' +
  '.catch(function(){' +
  'alert("Erro ao apagar. Verifique sua conexao e tente novamente.");' +
  'btn.disabled = false; btn.textContent = "Apagar";' +
  '});' +
  '});' +
  '});' +
