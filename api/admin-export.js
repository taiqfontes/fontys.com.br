...
function buildDiagnosticoDoc(d) {
  ...
  children.push(pKV('Perfil dominante', nomePerfil(d.perfil_dominante)));
  ...
  if (respostas.scores) {
    ...
    Object.entries(respostas.scores).sort((a, b) => b[1] - a[1]).forEach(([k, v]) => {
      rows.push(new TableRow({ children: [cell(nomePerfil(k) || humanize(k)), cell(v)] }));
    });
  ...
  const grupos = {};
  linhas.forEach(l => { (grupos[l.grupo] = grupos[l.grupo] || []).push(l); });

  Object.keys(grupos).forEach(grupo => {
    children.push(pHeading(grupo, 1));
    grupos[grupo].forEach(l => {
      children.push(new Paragraph({ spacing: { after: 20 }, children: [new TextRun({ text: l.pergunta, color: SLATE, italics: true })] }));
      children.push(new Paragraph({ spacing: { after: 160 }, children: [new TextRun({ text: l.resposta, bold: true })] }));
    });
  });
